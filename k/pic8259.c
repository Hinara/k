...
void pic_init(u8 offset1, u8 offset2)
{
	...
	outb(PIC1_DATA, 1 << IRQ_CASCADE);	// ICW3: tell Master PIC that there is a slave PIC at IRQ2
	outb(PIC2_DATA, IRQ_CASCADE);		// ICW3: tell Slave PIC its cascade identity
...
void irq_handler(struct x86_registers *regs, u32 interrupt, u32 code)
{
	(void) code;
	(void) regs;
	u8 irqno = 0;
